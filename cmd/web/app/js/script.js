function initApp() {
  ...
  const app = {
    ...
    async loadProducts() {
      const response = await fetch(`http://${ip}:${this.url}/v1/api/item-types`)
      const data = await response.json();
    ...
    async loadOrders() {
      this.orders = [];
      this.lineItems = [];
      const response = await fetch(`http://${this.url}/v1/fulfillment-orders`)
      const data = await response.json();
      this.orders = data.orders;
      console.log("orders loaded", this.orders);
    },
    async createOrder(order) {
      const response = await fetch(`http://${this.url}/v1/api/orders`, {
        method: 'POST',
        headers: {
